...
def main(spark, userID):
    ...
    tracks = spark.read.parquet(f'hdfs:/user/bm106_nyu_edu/1004-project-2023/tracks_train_small.parquet', schema="recording_msid STRING, artist_name STRING, track_name STRING, recording_mbid STRING")
    interactions = spark.read.parquet(f'hdfs:/user/bm106_nyu_edu/1004-project-2023/interactions_train_small.parquet', schema="user_id STRING, recording_msid STRING, timestamp STRING")
    ...
    print('Finished Count By User ID')
    interactions_tracks = interactions_tracks.orderBy(F.rand(seed=2023))
    ...
    df_val = interactions_tracks.alias("df").join(df_train.alias("tr"), (col("df.user_id") == col("tr.user_id")) & (col("df.row_number") == col("tr.row_number")), "leftanti")
    print('Finished Generating Train and Val DF')
    df_train.show(10)
    df_val.show(10)
    df_train.write.parquet("hdfs:/user/hk3820_nyu_edu/interactions_train.parquet")
    print('Finished Writing Train')
    df_val.write.parquet("hdfs:/user/hk3820_nyu_edu/interactions_val.parquet")
